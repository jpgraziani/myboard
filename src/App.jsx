...
function PinScreen({ onUnlock }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080B10', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: "'IBM Plex Mono', monospace", padding: '0 40px' }}>
      ...
      <div style={{ fontSize:10, letterSpacing:4, color:'rgba(255,255,255,0.25)', marginBottom:6, textTransform:'uppercase' }}>MY BOARD</div>
      <div style={{ fontSize:22, fontWeight:700, color:'#fff', letterSpacing:-0.5, marginBottom:8 }}>Enter PIN</div>
      <div style={{ fontSize:11, color: error ? '#FF4D6D' : 'rgba(255,255,255,0.25)', marginBottom:36, letterSpacing:1, transition:'color 0.2s' }}>{error ? 'INCORRECT PIN' : 'ENTER 4-DIGIT CODE'}</div>
      <div style={{ display:'flex', gap:16, marginBottom:48, animation: shake ? 'shake 0.5s ease' : 'none' }}>
        {[0,1,2,3].map(i => (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:12, width:'100%', maxWidth:280 }}>
        {keys.map((key, i) => (
          <button key={i} onClick={() => { if (key === '⌫') handleDelete(); else if (key !== '') handleKey(key) }} style={{ height:68, borderRadius:14, background: key==='' ? 'transparent' : key==='⌫' ? 'rgba(255,77,109,0.08)' : 'rgba(255,255,255,0.04)', border: key==='' ? 'none' : key==='⌫' ? '1px solid rgba(255,77,109,0.2)' : '1px solid rgba(255,255,255,0.07)', color: key==='⌫' ? '#FF4D6D' : '#fff', fontSize: key==='⌫' ? 18 : 22, fontWeight:700, cursor: key==='' ? 'default' : 'pointer', fontFamily:'inherit', letterSpacing:-0.5 }}>{key}</button>
        ))}
      </div>
      <div style={{ marginTop:40, fontSize:10, color:'rgba(255,255,255,0.1)', letterSpacing:2 }}>PROTECTED · MY BOARD</div>
    </div>
  )
}
...
function ListForm({ initial, submitLabel, onSubmit, onDelete, onClearAll }) {
  ...
  return (
    <>
      ...
      {onClearAll && <button onClick={onClearAll} style={{ width:'100%', padding:'12px 0', background:'transparent', border:'1px solid rgba(255,77,109,0.2)', borderRadius:10, color:'rgba(255,77,109,0.6)', fontSize:11, fontWeight:700, letterSpacing:1.5, cursor:'pointer', fontFamily:'inherit' }}>CLEAR ALL ITEMS</button>}
    </>
  )
}
...
function SwipeCard({ card, listColor, pri, onToggle, onEdit, onDelete, dragHandleProps, isDragging }) {
  ...
  function onTouchMove(e) {
    ...
    setSwipeX(Math.max(-THRESHOLD, Math.min(THRESHOLD, dx)))
  }
  function onTouchEnd() {
    setIsSwiping(false)
    if (Math.abs(swipeX) > THRESHOLD * 0.7) onDelete()
    else setSwipeX(0)
  }
  ...
  return (
    <div style={{ position:'relative', overflow:'hidden', borderRadius:9, opacity: isDragging ? 0.3 : 1 }}>
      {/* Right-swipe reveals DELETE on left */}
      <div style={{ position:'absolute', inset:0, background:`rgba(255,77,109,${showDelete && !isLeft ? swipeProgress * 0.25 : 0})`, display:'flex', alignItems:'center', paddingLeft:16, borderRadius:9, transition: isSwiping ? 'none' : 'background 0.2s' }}>
      ...
      {/* Left-swipe reveals DELETE on right */}
      <div style={{ position:'absolute', inset:0, background:`rgba(255,77,109,${showDelete && isLeft ? swipeProgress * 0.25 : 0})`, display:'flex', alignItems:'center', justifyContent:'flex-end', paddingRight:16, borderRadius:9, transition: isSwiping ? 'none' : 'background 0.2s' }}>
        <span style={{ fontSize:11, color:'#FF4D6D', fontWeight:700, letterSpacing:1.5, opacity: showDelete && isLeft ? swipeProgress : 0, transition: isSwiping ? 'none' : 'opacity 0.2s' }}>DELETE ⟶</span>
      </div>
      ...
      <div
        ...
        style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 12px', background: card.done ? 'rgba(255,255,255,0.01)' : 'rgba(255,255,255,0.04)', border:`1px solid ${card.done ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.07)'}`, borderLeft:`2px solid ${card.done ? 'rgba(255,255,255,0.06)' : pri.color}`, borderRadius:9, transition: isSwiping ? 'none' : 'transform 0.25s cubic-bezier(0.25,1,0.5,1)', transform:`translateX(${swipeX}px)`, position:'relative', zIndex:1, userSelect:'none' }}
      >
        {/* Drag handle */}
        <div {...dragHandleProps} style={{ display:'flex', flexDirection:'column', gap:3, padding:'0 4px', cursor:'grab', flexShrink:0 }}>
          {[0,1,2].map(i => <div key={i} style={{ width:14, height:2, borderRadius:1, background:'rgba(255,255,255,0.2)' }} />)}
        </div>
...
function SortableCard({ card, listColor, pri, onToggle, onEdit, onDelete }) {
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <SwipeCard
        card={card} listColor={listColor} pri={pri}
...
export default function App() {
  ...
  // dnd-kit sensors — touch with 8px movement tolerance so tap still works
  ...
  function clearAllCards(listId)     { setLists(ls=>ls.map(l=>l.id!==listId?l:{...l,cards:[]})) }
  ...
  function handleDragEnd({ active, over }) {
    ...
    const isListDrag = lists.some(l => l.id === activeId)
    if (isListDrag) {
      setLists(ls => {
        const oldIdx = ls.findIndex(l => l.id === activeId)
        const newIdx = ls.findIndex(l => l.id === overId)
        return arrayMove(ls, oldIdx, newIdx)
      })
      return
    }

    // Dragging a card — find source and destination lists
    let sourceListId = null, destListId = null
    for (const l of lists) {
      if (l.cards.some(c => c.id === activeId)) sourceListId = l.id
      if (l.cards.some(c => c.id === overId) || l.id === overId) destListId = l.id
    }
    ...
    <div ref={boardRef} style={{ minHeight:'100vh', height:'100vh', overflowY:'auto', background:'#080B10', fontFamily:"'IBM Plex Mono', 'Courier New', monospace", color:'#fff' }}>
              ...
              {lists.map((list, li) => {
                ...
                return (
                  <SortableList key={list.id} list={list} isDragging={activeId===list.id}>
                    {({ dragHandleProps }) => (
                      <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:14, overflow:'hidden', animation:`fadeIn 0.3s ease ${li*0.06}s both` }}>
                        {/* List header */}
                        <div style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 14px', borderBottom:isCol?'none':'1px solid rgba(255,255,255,0.05)' }}>
                          ...
                          <div {...dragHandleProps} style={{ width:38, height:38, borderRadius:10, flexShrink:0, background:`${list.color}18`, border:`1px solid ${list.color}44`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:17, color:list.color, cursor:'grab', touchAction:'none' }}>{list.icon}</div>
      ...
      {modal?.type==='editList' && (
        <Modal title="Edit List" onClose={()=>setModal(null)}>
          <ListForm initial={modal.list} submitLabel="Save Changes" onSubmit={data=>{updateList(modal.list.id,data);setModal(null)}} onDelete={()=>{removeList(modal.list.id);setModal(null)}} onClearAll={()=>{clearAllCards(modal.list.id);setModal(null)}}/>
        </Modal>
      )}
    </div>
  )
