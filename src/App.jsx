...
const DEFAULT_LISTS = [
  {
    ...
    color: '#00D4AA',
    cards: [
      { id: 's1', text: 'Highlighters (4 colors)', done: false, priority: 'medium' },
      { id: 's2', text: 'Index cards', done: false, priority: 'low' },
    ],
  },
  {
    id: 'study',
    ...
    color: '#7B8CFF',
    cards: [
      { id: 'n1', text: 'Review pharmacology Ch. 3', done: false, priority: 'high' },
      { id: 'n2', text: 'NCLEX practice — cardiac questions', done: false, priority: 'high' },
    ],
  },
  {
    id: 'todo',
    ...
    color: '#FFB547',
    cards: [
      { id: 't1', text: 'Submit lab report by Friday', done: false, priority: 'high' },
    ],
  },
]
...
// ─── Supabase helpers ────────────────────────────────────────────────────────
// We store the entire board as a single JSON row in a table called "boards".
// Row has: id (text, primary key = 'default'), data (jsonb)
...
async function saveToSupabase(lists) {
  await supabase
    .from('boards')
    .upsert({ id: 'default', data: lists }, { onConflict: 'id' })
}
    ...
    <>
      <textarea autoFocus value={text} onChange={e => setText(e.target.value)} rows={3}
        ...
        style={{
          ...
          border: `1px solid ${listColor}55`, borderRadius: 10,
          padding: '12px 14px', color: '#fff', fontSize: 15, lineHeight: 1.5,
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {Object.entries(PRIORITY_CONFIG).map(([key, cfg]) => (
            <button key={key} onClick={() => setPriority(key)} style={{
              ...
              background: priority === key ? `${cfg.color}22` : 'rgba(255,255,255,0.04)',
              border: `1px solid ${priority === key ? cfg.color : 'rgba(255,255,255,0.08)'}`,
              borderRadius: 8, color: priority === key ? cfg.color : 'rgba(255,255,255,0.3)',
              fontSize: 11, fontWeight: 700, letterSpacing: 1.5, cursor: 'pointer', fontFamily: 'inherit',
            }}>{cfg.label}</button>
          ))}
        </div>
      </div>
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={() => text.trim() && onSubmit({ text: text.trim(), priority })} style={{
          flex: 1, padding: '14px 0', background: listColor, border: 'none', borderRadius: 10,
          color: '#000', fontSize: 13, fontWeight: 700, cursor: 'pointer', letterSpacing: 0.5,
        }}>{submitLabel}</button>
        {onDelete && (
          <button onClick={onDelete} style={{
            padding: '14px 18px',
            background: 'rgba(255,77,109,0.12)', border: '1px solid #FF4D6D33',
            borderRadius: 10, color: '#FF4D6D', fontSize: 13, cursor: 'pointer',
          }}>Delete</button>
        )}
      </div>
    </>
  ...
  return (
    <>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {ICONS.map(i => (
            <div key={i} onClick={() => setIcon(i)} style={{
              width: 36, height: 36, borderRadius: 8, cursor: 'pointer',
              background: icon === i ? `${color}22` : 'rgba(255,255,255,0.04)',
              border: `1px solid ${icon === i ? color : 'rgba(255,255,255,0.08)'}`,
              color: icon === i ? color : 'rgba(255,255,255,0.3)',
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={() => title.trim() && onSubmit({ title: title.trim(), color, icon })} style={{
          flex: 1, padding: '14px 0', background: color, border: 'none', borderRadius: 10,
...
function ChartsView({ lists }) {
  const donutData = lists.map(l => ({
    name: l.title, value: l.cards.filter(c => !c.done).length,
    color: l.color, icon: l.icon, total: l.cards.length,
  })).filter(d => d.total > 0)
  ...
  const completionData = lists.map(l => ({
    name: l.title, color: l.color,
    done: l.cards.filter(c => c.done).length,
    pending: l.cards.filter(c => !c.done).length,
    pct: l.cards.length > 0 ? Math.round((l.cards.filter(c => c.done).length / l.cards.length) * 100) : 0,
  }))
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <div style={{ position: 'relative', width: 90, height: 90, flexShrink: 0 }}>
            <ResponsiveContainer width={90} height={90}>
              <PieChart>
                <Pie data={[{ v: totalDone }, { v: Math.max(totalItems - totalDone, 0) }]} dataKey="v"
                  innerRadius={28} outerRadius={40} startAngle={90} endAngle={-270} strokeWidth={0}>
                  <Cell fill="#00D4AA" />
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: -0.5 }}>
              {totalDone} <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)', fontWeight: 400 }}>/ {totalItems} done</span>
            </div>
        ...
        {donutData.every(d => d.value === 0) ? (
          ...
          <>
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie data={donutData.filter(d => d.value > 0)} dataKey="value"
                  innerRadius={50} outerRadius={80} paddingAngle={3} labelLine={false} label={renderLabel}>
                  {donutData.filter(d => d.value > 0).map((d, i) => <Cell key={i} fill={d.color} />)}
                </Pie>
                <Tooltip contentStyle={{ background: '#0E1117', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontFamily: 'IBM Plex Mono', fontSize: 11 }} />
              </PieChart>
            </ResponsiveContainer>
...
export default function App() {
  ...
  // Save to Supabase whenever lists change (debounced 800ms)
  useEffect(() => {
    if (loading) return
    setSaving(true)
    const t = setTimeout(() => {
      saveToSupabase(lists).finally(() => setSaving(false))
    }, 800)
  ...
  function updateCard(listId, cardId, patch) {
    setLists(ls => ls.map(l => l.id !== listId ? l : {
      ...l, cards: l.cards.map(c => c.id !== cardId ? c : { ...c, ...patch })
    }))
  }
  ...
  function addCard(listId, card) {
    setLists(ls => ls.map(l => l.id !== listId ? l : { ...l, cards: [...l.cards, card] }))
  }
    ...
    <div style={{ minHeight: '100vh', background: '#080B10', fontFamily: "'IBM Plex Mono', 'Courier New', monospace", color: '#fff' }}>
      ...
      <div style={{ padding: '24px 18px 0', position: 'sticky', top: 0, zIndex: 10, background: 'linear-gradient(180deg,#080B10 75%,transparent 100%)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          ...
          <button onClick={() => setModal({ type: 'newList' })} style={{
            background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)',
            borderRadius: 10, padding: '10px 14px', color: 'rgba(255,255,255,0.65)',
            fontSize: 11, fontWeight: 700, cursor: 'pointer', letterSpacing: 1.5, fontFamily: 'inherit',
          }}>+ LIST</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 4 }}>
          {[['board', '▦  BOARD'], ['charts', '◉  CHARTS']].map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)} style={{
              ...
              borderRadius: 7, color: tab === key ? '#fff' : 'rgba(255,255,255,0.3)',
              fontSize: 10, fontWeight: 700, letterSpacing: 2, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s',
            }}>{label}</button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', marginBottom: 20, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, overflow: 'hidden' }}>
          {[
            { label: 'LISTS',   value: lists.length,  color: '#7B8CFF' },
            { label: 'PENDING', value: totalPending,   color: '#00D4AA' },
            { label: 'URGENT',  value: totalHigh,      color: '#FF4D6D' },
          ].map((s, i) => (
              ...
              <div key={list.id} style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden', animation: `fadeIn 0.3s ease ${li * 0.06}s both` }}>
                ...
                {!isCol && (
                  <div style={{ padding: '10px 12px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {list.cards.length === 0 && (
                      <div style={{ textAlign: 'center', padding: '18px 0', color: 'rgba(255,255,255,0.12)', fontSize: 11, letterSpacing: 2 }}>NO ITEMS</div>
                    )}
                    {list.cards.map(card => {
      ...
      {modal?.type === 'editCard' && (() => {
        ...
        return (
          <Modal title="Edit Item" onClose={() => setModal(null)}>
            <CardForm initial={modal.card} listColor={list?.color || '#fff'} submitLabel="Save Changes"
              onSubmit={data => { updateCard(modal.listId, modal.card.id, data); setModal(null) }}
              onDelete={() => { removeCard(modal.listId, modal.card.id); setModal(null) }}
            />
          </Modal>
        )
      })()}
      ...
      {modal?.type === 'addCard' && (
        <Modal title={`Add to ${modal.list.title}`} onClose={() => setModal(null)}>
          <CardForm listColor={modal.list.color} submitLabel="Add Item"
            onSubmit={data => { addCard(modal.list.id, { id: genId(), done: false, ...data }); setModal(null) }}
          />
        </Modal>
      )}

      {modal?.type === 'newList' && (
        <Modal title="New List" onClose={() => setModal(null)}>
          <ListForm submitLabel="Create List" onSubmit={data => { addList(data); setModal(null) }} />
        </Modal>
      )}

      {modal?.type === 'editList' && (
        <Modal title="Edit List" onClose={() => setModal(null)}>
          <ListForm initial={modal.list} submitLabel="Save Changes"
